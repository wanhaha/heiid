...
class UserController extends BaseController {
    ...
    public function reg(){
       ...
       // file_put_contents('test.txt',$username.$password.$password2,FILE_APPEND);
        $data = $this->userLogic->reg($username,$password,$password2,$head_pic);
        exit(json_encode($data));
    }
    ...
    public function updateUserInfo(){
        if(IS_POST){
            ...
            I('post.describe') ? $post['describe'] = I('post.describe') : false;  //诚信宣言
        ...
        if(IS_POST){
            ...
            if(check_mobile($username)){
                if(!$user = get_user_info($username,2))
                    exit(json_encode(array('status'=>-1,'msg'=>'账号不存在','result'=>'')));
                $check_code = $logic->sms_code_verify($username,$code,$unique_id);
                if($check_code['status'] != 1)
                    exit(json_encode(array('status'=>-1,'msg'=>$check_code['msg'],'result'=>'')));
                $pass = true;
            }
            ...
            if($user['user_id'] > 0 && $pass)
                $data = $logic->uppassword($user['user_id'],$new_password,$confirm_password,false); // 获取用户信息
            if($data['status'] != 1){
                exit(json_encode(array('status'=>-1,'msg'=> '账号不存在','result'=>'')));
            }else{
                exit(json_encode(array('status'=>1,'msg'=> '密码修改成功','result'=>$data['result'])));
            }
    ...
    public function return_goods_status()
    {
        ...
        if(!empty($return_goods))        
            exit(json_encode(array('status'=>-1,'msg'=>'已经在申请退货中..','result'=>$return_goods['id']))); 
             ...
             exit(json_encode(array('status'=>1,'msg'=>'可以去申请退货','result'=>$return_goods['id'])));
    }
    /**
     * 申请退货
     */
