...
class PinglunController extends BaseController {
    ...
    public function detail(){
        $id = I('get.id');
        $res = M('pinglun')->where(array('id'=>$id))->find();
        if(!$res){
        ...
        $reply = M('pinglun')->where(array('parent_id'=>$id))->select(); // 评论回复列表
    ...
    public function del(){
        $id = I('get.id');
        $row = M('pinglun')->where(array('id'=>$id))->delete();
        if($row){
            $this->success('删除成功');
        }else{
            $this->error('删除失败');
    ...
    public function op(){
        ...
        if($type == 'del'){
            ...
            $row = M('pinglun')->where($where)->delete();
//            exit(M()->getLastSql());
        }
        if($type == 'show'){
            $row = M('pinglun')->where($where)->save(array('is_show'=>1));
        }
        if($type == 'hide'){
            $row = M('pinglun')->where($where)->save(array('is_show'=>0));
        }
        if(!$row)
            $this->error('操作失败');
        $this->success('操作成功');
    ...
    public function ajaxindex(){
       ...
       // $count = $model->where(array('parent_id'=>0))->count();
        ...
        $sql = "SELECT COUNT(1) as total_count FROM __PREFIX__pinglun WHERE parent_id=0".$where;
        $count = M()->query($sql);
//        exit(M()->getLastSql());
        ...
        $sql = "SELECT * FROM __PREFIX__pinglun WHERE parent_id=0".$where.' ORDER BY add_time DESC LIMIT '.$Page->firstRow.','.$Page->listRows;
//        exit($sql);
...
//        $comment_list = $model->where(array('parent_id'=>0))->order('add_time DESC')->limit($Page->firstRow.','.$Page->listRows)->select();
        if(!empty($comment_list))
        {
            $goods_id_arr = get_arr_column($comment_list, 'sid');
            $goods_list = M('piyao')->where("id in (".  implode(',', $goods_id_arr).")")->getField("id,title");
        }
        $this->assign('goods_list',$goods_list);
        $this->assign('comment_list',$comment_list);
