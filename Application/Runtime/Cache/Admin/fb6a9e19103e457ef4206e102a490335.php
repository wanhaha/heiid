					...
					<div class="panel-body">
						<nav class="navbar navbar-default">
							<div class="collapse navbar-collapse">
								<form action="<?php echo U('Pinglun/index');?>" id="search-form2" class="navbar-form form-inline" role="search" method="post">
							...
							<div class="table-responsive">
								<table class="table table-bordered table-hover">
									...
									<?php if(is_array($comment_list)): $i = 0; $__LIST__ = $comment_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr>
											...
											<td class="text-left"><a target="_blank" href="#"><?php echo ($goods_list[$list[id]]); ?></a></td>
											<td class="text-center">
												<img width="20" height="20" src="/Public/images/<?php if($list[is_show] == 1): ?>yes.png<?php else: ?>cancel.png<?php endif; ?>" onclick="changeTableVal('pinglun','id','<?php echo ($list["id"]); ?>','is_show',this)"/>
											</td>
											<td class="text-center"><?php echo (date('Y-m-d H:i:s',$list["add_time"])); ?></td>
											<td class="text-center"><?php echo ($list["ip_address"]); ?></td>

											<td class="text-center">
												<a href="<?php echo U('Admin/pinglun/detail',array('id'=>$list[id]));?>" data-toggle="tooltip" title="" class="btn btn-primary" data-original-title="编辑"><i class="fa fa-eye"></i></a>
												<a href="javascript:void(0);" data-href="<?php echo U('Admin/pinglun/del',array('id'=>$list[id]));?>" onclick="del('<?php echo ($list[id]); ?>',this)" id="button-delete6" data-toggle="tooltip" title="" class="btn btn-danger" data-original-title="删除"><i class="fa fa-trash-o"></i></a>
											</td>
...
<script>
	...
	function ajax_get_table(tab,page){
		...
		$.ajax({
			type : "POST",
			url:"/index.php/Admin/Pinglun/ajaxindex/p/"+page,//+tab,
