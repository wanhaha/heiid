...
class ChengxinController extends BaseController {
    ...
    public function daikuan(){
        ...
        foreach($list as $k=>$v){
            $list[$k]['status'] = empty($list[$k]['status'])?'未审核':'已审核';
            $list[$k]['tel'] = empty($list[$k]['tel'])?$list[$k]['phone']:$list[$k]['tel'];
            $list[$k]['uid'] = empty($list[$k]['uid'])?'管理员':get_user_info($list[$k]['uid'])['nickname'];
    ...
    public function article(){
        ...
        if(I('GET.id')){
           $article_id = I('GET.id');
           $info = D('heiid')->where('id='.$article_id)->find();
        }
    ...
    public function heiidHandle(){
        $data = I('post.');
        $data['name'] = empty($data['name'])?$data['qyname']:$data['name'];
        $data['phone'] = empty($data['phone'])?$data['tel']:$data['phone'];
        $data['uid'] = empty($data['uid'])?1:$data['uid'];
        ...
        if($data['act'] == 'edit'){
            $r = D('heiid')->where('id='.$data['id'])->save($data);
        }
        
        if($data['act'] == 'del'){

        	$r = D('heiid')->where('id='.$data['article_id'])->delete();
        	if($r) exit(json_encode(1));       	
        }
        $referurl = isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : U('Admin/Heiid/index');
        if($r){
