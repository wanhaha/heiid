...
class UsersLogic extends RelationModel
{
     ...
     * @return array
     */
    public function reg($username,$password,$password2,$head_pic){
    	$is_validated = 0 ;
        ...
        return array('status'=>1,'msg'=>'注册成功,请完善信息','result'=>$user);
    }
...
//        $type = I('get.type',0);
     ...
     * @param $confirm_password 确认新 密码
     */
    public function uppassword($user_id,$new_password,$confirm_password,$is_update=true){
        $data = $this->get_info($user_id);
        $user = $data['result'];
        if(strlen($new_password) < 6)
            return array('status'=>-1,'msg'=>'密码不能低于6位字符','result'=>'');
        ...
        if(!$row)
            return array('status'=>-1,'msg'=>'修改失败','result'=>'');
        return array('status'=>1,'msg'=>'修改成功','result'=>$user);
    }
    /**
     * 取消订单
     */
