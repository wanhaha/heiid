 ...
 */        
    ...
    // 分销演示 二维码地址
    public function distribut_qr_code()
    {
        $wx_user_id = M('users')->where("oauth = 'weixin' and user_id >=175")->order('user_id desc')->getField('user_id');
        //$rand_keys = array_rand($wx_user_id, 1);        
        $url = "http://www.tp-shop.cn/index.php?m=Mobile&c=Index&a=index&first_leader=".$wx_user_id;
        $url2 = "/index.php?m=Home&c=Index&a=qr_code&data=".urlencode($url);
        exit($url2);
    }
